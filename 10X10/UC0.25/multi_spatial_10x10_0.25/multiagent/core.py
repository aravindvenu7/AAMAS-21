...
class World(object):
    def __init__(self):
        ...
        river_dist = np.zeros((10,10))
        boundary_dist =  np.zeros((10,10))
        road_dist =  np.zeros((10,10))
        ...
        river_dist = river_dist.reshape((100))
        road_dist = road_dist.reshape((100))
        boundary_dist = boundary_dist.reshape((100))           
        ...
        river_dist = np.array(river_dist[:100])
        ...
        boundary_dist = boundary_dist[:100]
        ...
        afr = np.reshape(afr,(10,10))
        pfr = np.reshape(pfr,(10,10))

        self.afr_temp = afr/100.0



        self.pfr_temp = pfr/100.0
    ...
    def reached_poacher_int_target(self,agent):
        ...
        if(agent.state.p_pos[0] == agent.target[0] and agent.state.p_pos[1] == agent.target[1]):
          agent.reached_int_target = True
    ...
    def scripted_action(self, agent):
      ...
      if(agent.reached_int_target and agent.target is not None): # UPDATE FOR INTERMEDIATE TARGETS


        agent.targets.append(agent.target.copy())
        ...
        ##################################################################
        agent.reward = agent.reward + 1
        self.poacher_policy[int(agent.target[0])][int(agent.target[1])] = 0.0  
      ...
      if(agent.name == "poacher"):
        ...
        if(agent.is_caught):
        ...
        elif(agent.is_flee or agent.reached_target):
          ...
          agent.action.u[0] = agent.target[0]
          agent.action.u[1] = agent.target[1]
    ...
    def update_agent_state(self,agent): 
        ...
        if(agent.name == "poacher" and not agent.is_flee and not agent.reached_target):
