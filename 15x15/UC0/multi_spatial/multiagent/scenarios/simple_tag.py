        ...
        for agentp in world.poachers:
          ...
          if(np.sqrt(np.sum(np.square(delta_pos))) == 0):
            ...
            if(agent1.action.warn):
              agentp.is_flee  = True
              '''if(key == 1):
                print("prem flee activated for poacher,drone",agentp.id,agent1.id)
                print("poacher pos",agentp.state.p_pos)
                print("poacher flee status", agentp.is_flee)
                print("time of flee", world.time)'''
    ...
    def escaped(self,world,agent):
      ...
      if(agent.is_caught):
        ...
        if(agent.reached_target or agent.is_flee):
          if(posp[0] == 0 or posp[0] == (world.gridsize-1) or posp[1] == 0 or posp[1] == (world.gridsize-1) or posp[0] == -99 or posp[1] == -99):
            return True
          else:
            return False  
        else:
