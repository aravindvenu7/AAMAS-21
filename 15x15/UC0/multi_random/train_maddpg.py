...
def load_models():
      ...
      maddpg.agents[0].policy.load_state_dict(torch.load("maddpg_models/drone_policy20.pth"))
      maddpg.agents[0].target_policy.load_state_dict(torch.load("maddpg_models/drone_tpolicy20.pth"))     
      maddpg.agents[0].critic.load_state_dict(torch.load("maddpg_models/drone_critic20.pth"))      
      maddpg.agents[0].target_critic.load_state_dict(torch.load("maddpg_models/drone_tcritic20.pth"))
        
      maddpg.agents[1].policy.load_state_dict(torch.load("maddpg_models/ranger_policy20.pth"))
      maddpg.agents[1].target_policy.load_state_dict(torch.load("maddpg_models/ranger_tpolicy20.pth"))      
      maddpg.agents[1].critic.load_state_dict(torch.load("maddpg_models/ranger_critic20.pth"))      
      maddpg.agents[1].target_critic.load_state_dict(torch.load("maddpg_models/ranger_tcritic20.pth"))
      ...
      for episode in range(20):
          ...
          for step in range(100):
              ...
              for i in range(len(dstates)):
                ...
                action = maddpg.agents[0].step(Variable(torch.from_numpy(dstates[i]).float().unsqueeze(0)).to('cuda')) #,hidden_states_d[i],cell_states_d[i])
                #hidden_states_d[i] = h.to(device)
                #cell_states_d[i] =   c.to(device)
                d_actions.append(action)
              for i in range(len(rstates)):
                action = maddpg.agents[1].step(Variable(torch.from_numpy(rstates[i]).float().unsqueeze(0)).to('cuda')) #,hidden_states_r[i],cell_states_r[i])
          ...
          eval_rewards[episode] = episode_reward_drone + episode_reward_ranger
        
      #print(len(rewards))
      np.save("rewards_maddpg.npy", eval_rewards)
      #return rewards
