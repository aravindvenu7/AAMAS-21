...
def plot_rewards(rewards_d,rewards_r,episode, counts, winrates):
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.savefig('re_dqn_data/rewards_dqn_ep' + str(episode) + '.png')
    plt.close()

    plt.plot(winrates)
    plt.xlabel('Episode*100')
    plt.ylabel('Winrate')
    plt.savefig('re_dqn_data/winrates_dqn_ep' + str(episode) + '.png')
    plt.close()

    plt.xlabel('re_dqn_data/Visitation counts')
    plt.imshow(counts[-1])
    plt.savefig('re_dqn_data/v_counts_dqn_ep' + str(episode) + '.png')
    plt.close()

    plt.xlabel('Attacker probability acc to coverage')
    plt.imshow(coverage)
    plt.savefig('re_dqn_data/attacker_prob_dqn_ep' + str(episode) + '.png')
    plt.close()
    ...
    plt.imshow(x)
    plt.savefig('re_dqn_data/coverage_diff_dqn_ep' + str(episode) + '.png')
    plt.close()
    ...
    if(episode > 0 ):
      ...
      plt.imshow(y)
      plt.savefig('re_dqn_data/counts_diff_dqn_ep' + str(episode) + '.png')
      plt.close()
...
def load_models():
      ...
      agent_d.online_network.load_state_dict(torch.load("models/retrained_drone_online5000.pth"))
      agent_d.target_network.load_state_dict(torch.load("models/retrained_drone_target5000.pth"))      
      agent_r.online_network.load_state_dict(torch.load("models/retrained_ranger_online5000.pth"))      
      agent_r.target_network.load_state_dict(torch.load("models/retrained_ranger_target5000.pth"))
...
def retrain(dralloc, ralloc, palloc, agent_d, agent_r):
      ...
      for episode in range(100001):
          ...
          for i in range(5):
            inits.append(np.array([init0[i], init1[i]]))'''
          drone_inits = dralloc[episode]
          ranger_inits = ralloc[episode]  


        
          poacher_inits = palloc[episode]
          ...
          for step in range(100):
              ...
              if done[0] or step == 99:
                ...
                if(episode%500==0): 
                      ...
                      plt.savefig('re_dqn_data/attacker_counts_dqn_ep' + str(episode) + '.png')
                      plt.close()
                      plt.xlabel('Attacker probs')
                      plt.imshow(env.world.action_probs_display)
                      plt.savefig('re_dqn_data/attacker_probs_dqn_ep' + str(episode) + '.png')
                      plt.close()
          ...
          if(episode%2000 == 0):
                np.save('re_dqn_data/winrate_dqn_ep' + str(episode),np.array(winrates))
                np.save('re_dqn_data/rewardsd_dqn_ep' + str(episode),np.array(rewards_d))
                np.save('re_dqn_data/rewardsr_dqn_ep' + str(episode),np.array(rewards_r))
          ...
          if(episode%5000 == 0 and episode > 0):
            torch.save(agent_d.online_network.state_dict(),"models/retrained_drone_online" + str(episode) + ".pth")
            torch.save(agent_d.target_network.state_dict(),"models/retrained_drone_target" + str(episode) + ".pth")
            torch.save(agent_r.online_network.state_dict(),"models/retrained_ranger_online" + str(episode) + ".pth")
            torch.save(agent_r.target_network.state_dict(),"models/retrained_ranger_target" + str(episode) + ".pth")
            #if(episode > 0):
            #  evaluate()
