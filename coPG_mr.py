...
def main():
      ...
      for j in range(gridsize):
        for k in range(gridsize):
          for l in range(gridsize):
              p_allocations.append([np.array([i,j]), np.array([k, l])])    
    ...
    for i in range(gridsize):
      for j in range(gridsize):
        for k in range(gridsize):
          for l in range(gridsize):
              poacher_embeddings.append([i/14, j/14, k/14, l/14])
    poacher_embeddings = np.array(poacher_embeddings) 
    p_a = []
    for i in range(50625):
     p_a.append(i) 
    p_indexes =  random.sample(p_a, 10000)  
    p_e = []
    p_alloc_temp = []
    for item in p_indexes:
      
      p_e.append(poacher_embeddings[item])
      p_alloc_temp.append(p_allocations[item])
      
    
    poacher_embeddings = np.array(p_e)
    p_allocations = p_alloc_temp.copy()
    ...
    #model = RLAgent(config, defender_embeddings, 128, 50)  #embed_Reinforce(agent)
    #poacher = RLAgent(configp, poacher_embeddings, 32, 2)
    
  

    cgdagent = CGDAgent(config, configp, defender_embeddings, poacher_embeddings, 128, 50, 32, 4)
    ...
    for trial in range(5000):
      ...
      '''if(trial%10==0 and trial > 0):
          plot_rewards(defender_rewards,50)
          #plt.show()
          plot_rewards(poacher_rewards,50)
          #plt.show()'''


      if(len(poacher_rewards) == 5000):
          np.save('coPG_results/defender_rewardse5.npy', np.array(defender_rewards))
          np.save('coPG_results/poacher_rewardse5.npy', np.array(poacher_rewards))
          np.save('coPG_results/defender_allocationse5.npy', np.array(def_pos))
          np.save('coPG_results/poacher_allocationse5.npy', np.array(poacher_pos))
          np.save('coPG_results/defender_visitse5.npy', np.array(def_visits))
          np.save('coPG_results/poacher_visitse5.npy', np.array(poacher_visits))      
