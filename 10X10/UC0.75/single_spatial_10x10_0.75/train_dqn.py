...
def plot_rewards(rewards_d,rewards_r,episode, counts, winrates):
    ...
    counts1 = counts[-1].reshape((100))
    countrank1 = rankdata(counts1,method = 'ordinal') 
    countrank1 = (100 - countrank1) / 100.0  
    coverage = countrank1.reshape((10,10)) 
    
    if(episode > 0):
      counts2 = counts[-100].reshape((100))
      countrank2 = rankdata(counts2,method = 'ordinal') 
      countrank2 = (100 - countrank2) / 100.0  
      prev_coverage = countrank2.reshape((10,10))     
    else:
      prev_coverage = np.zeros((10,10))
...
def evaluate(defender_inits, poacher_inits, agent_d, agent_r, random = False):
      ...
      agent_d.online_network.load_state_dict(torch.load("uc_models/drone_online20000.pth"))
      agent_d.target_network.load_state_dict(torch.load("uc_models/drone_target20000.pth"))      
      agent_r.online_network.load_state_dict(torch.load("uc_models/ranger_online20000.pth"))      
      agent_r.target_network.load_state_dict(torch.load("uc_models/ranger_target20000.pth"))'''    
      ...
      return episode_reward, env.world.v_counts, env.world.poacher_counts
